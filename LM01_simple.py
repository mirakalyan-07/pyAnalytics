...
y #DV [score] #(y can 2 dim also : y.reshape((-1,1)))

#S3 - Model
model = LinearRegression()  
model.fit(x,y)  #creating blank linear regression model and inserting x and y values in it [type-1]

model = LinearRegression().fit(x,y) #[type-2] #2 lines into 1
...
y
x, y = np.array(x), np.array(y)#assigning x and y as arrays
...
x
model3 = sm.OLS(y,x)#ordinary least square
model3
results = model3.fit()
results
results.summary()
